...
/**
 * MFM1B: rotation-invariant bounded M10-R multi-field architecture proxy.
 *
 * Keeps MFM1A's recovered foundations:
 * - exact recovered 16x22 Integral mask;
 * - recovered MFM 4x6 / 24-region topology;
 * - recovered M10-R A-mode allocator downstream.
 *
 * MFM1B changes only the phone-domain proxy used to recognize positive
 * backlight assist. MFM1A assumed one signed upper/lower scene direction and
 * averaged all perimeter regions together. That can miss a bright source on
 * another side of the frame. MFM1B therefore:
 * - removes the lower-half term from the positive EV magnitude;
 * - evaluates the brightest of top/bottom/left/right against the inner field;
 * - adds an orientation-neutral Integral-vs-regional-median fallback;
 * - requires both bright and dark regional populations before positive assist.
 *
 * It remains an architecture proxy. Exact numerical parity with Leica's
 * unresolved 13-feature CA9 generator is NOT claimed.
 */
public final class M10RMfm1B {
    ...
    // Phone-domain live-grid calibration. Not a recovered Leica firmware constant.
    public static final double POSITIVE_GAIN = 1.50;
    ...
    public static final class Decision {
        ...
        public final double left4Y;
        public final double right4Y;
        public final double bottom6Y;
        public final double brightestSideY;
        ...
        public final double brightestSideVsInnerEv;
        ...
        Decision(boolean valid, boolean wouldApply, double recommendedEv,
                 ...
                 double sceneSpreadEv,
                 double center8Y, double lower12Y, double upper6Y,
                 double edge16Y, double inner8Y,
                 double left4Y, double right4Y, double bottom6Y, double brightestSideY,
                 double integralVsMedianEv, double integralVsCenterEv, double integralVsLowerEv,
                 double edgeVsInnerEv, double upperVsLowerEv, double brightestSideVsInnerEv,
            ...
            this.sceneSpreadEv=sceneSpreadEv;
            this.center8Y=center8Y; this.lower12Y=lower12Y; this.upper6Y=upper6Y;
            this.edge16Y=edge16Y; this.inner8Y=inner8Y;
            this.left4Y=left4Y; this.right4Y=right4Y; this.bottom6Y=bottom6Y;
            this.brightestSideY=brightestSideY;
            ...
            this.brightestSideVsInnerEv=brightestSideVsInnerEv;
    ...
    public static Decision evaluate(double[] cell) {
        ...
        double bottom6Y = regionRectMean(regions, 3, 4, 0, 6);
        double left4Y = regionRectMean(regions, 0, 4, 0, 1);
        double right4Y = regionRectMean(regions, 0, 4, 5, 6);
        double edge16Y = regionEdgeMean(regions, true);
        double inner8Y = regionEdgeMean(regions, false);
        double brightestSideY = Math.max(Math.max(upper6Y, bottom6Y), Math.max(left4Y, right4Y));
        ...
        double brightestSideVsInnerEv = log2(safe(brightestSideY) / safe(inner8Y));
        ...
        // MFM1B positive path.
        //
        // The magnitude is deliberately orientation-neutral. The old lower-half
        // term could cancel a true backlight when the bright source happened to
        // occupy that half of the sensor after phone rotation.
        double positiveBaseEv = Math.max(0.0,
                0.65 * integralVsMedianEv
                + 0.35 * integralVsCenterEv);

        // A single bright side is enough; averaging all four edges can dilute it.
        double sideGeometryConfidence = smoothstep(brightestSideVsInnerEv, 0.45, 1.20);

        // Strong Integral dominance is a direction-independent fallback for a
        ...
        double positiveCandidateEv =
                POSITIVE_GAIN * positiveBaseEv * positiveConfidence;

        // Negative moderation path is unchanged from MFM1A.
        ...
        return new Decision(valid, wouldApply, valid ? candidateEv : 0.0,
                ...
                sceneSpreadEv,
                center8Y, lower12Y, upper6Y, edge16Y, inner8Y,
                left4Y, right4Y, bottom6Y, brightestSideY,
                integralVsMedianEv, integralVsCenterEv, integralVsLowerEv,
                edgeVsInnerEv, upperVsLowerEv, brightestSideVsInnerEv,
