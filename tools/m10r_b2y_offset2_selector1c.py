    ...
    for ins in md.disasm(data[ENTRY:ENTRY+MAXSPAN],BASE+ENTRY):
        ...
        for op in ops:
            if op.type==ARM_OP_MEM and op.mem.base==ARM_REG_PC:
                ...
                if v is not None and 0x20000000<=v<0x21000000:mmio.append({"pc":hex(off),"value":hex(v)})
        ...
        if off>ENTRY+2 and ((ins.mnemonic=="pop" and "pc" in ins.op_str) or (ins.mnemonic=="bx" and ins.op_str.strip()=="lr")):
            break
    nearby=strings(data,ENTRY-0x300,ENTRY+0x700)
    result={"schema":"M10R_B2Y_OFFSET2_SELECTOR1C_V1","entry":hex(ENTRY),
            "instructions":insns,"calls":calls,"mmio_literals":mmio,"nearby_strings":nearby}
    a.out.parent.mkdir(parents=True,exist_ok=True);a.out.write_text(json.dumps(result,indent=2)+"\n")
    print("CALLS",json.dumps(calls,indent=2))
    print("MMIO",json.dumps(mmio,indent=2))
    print("STRINGS")
    for s in nearby:
        if any(k in s["text"].lower() for k in ("offset","b2y","noise","nr","color","chroma","iso")):
            print(s["offset"],s["text"])
    for x in insns:
        extra=[]
        if "literals" in x:extra.append("LIT="+",".join(z["pool"]+"->"+str(z["value"]) for z in x["literals"]))
        if "call_target" in x:extra.append("CALL="+x["call_target"])
        print(f"{x['offset']}: {x['mnemonic']:8s} {x['op_str']}"+((" ; "+" ; ".join(extra)) if extra else ""))
