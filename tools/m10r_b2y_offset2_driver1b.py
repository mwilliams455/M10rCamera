...
def main():
    ...
    for ins in md.disasm(data[ENTRY:ENTRY+MAXSPAN],BASE+ENTRY):
        ...
        if off>ENTRY+2 and ((ins.mnemonic=="pop" and "pc" in ins.op_str) or (ins.mnemonic=="bx" and ins.op_str.strip()=="lr")):
            break
    nearby=strings(data,ENTRY-0x700,ENTRY+0x1000)
    result={"schema":"M10R_B2Y_OFFSET2_DRIVER1B_V1","entry":hex(ENTRY),"instruction_count":len(out),
            "mmio_literals":mmio,"calls":calls,"store_sites":stores,"nearby_strings":nearby,"instructions":out}
    ...
    print("STRINGS")
    for s in nearby:
        if any(k in s["text"].lower() for k in ("b2y","noise","lum","nr","offset","filter","edge","chroma","color")):
