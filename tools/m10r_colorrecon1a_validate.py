#!/usr/bin/env python3
"""Fixed COLORRECON1A native validation. Public references only; no private RAWs."""
...
def oracle(taps,dst,tone,dg,post):
 ...
 lin=a.xform(taps[:,:3],dst);w=np.array([.2126,.7152,.0722]);lum=np.maximum(lin@w,1e-12);peak=np.maximum(lin.max(1),1e-12)
...
def main():
 ...
 q=(inp/'source/m10rRender.cpp').read_text();r=load('rp',repo/'patches/fix-m10r-render1r-tonecal1a.py').patch_cpp(q);s=load('sp',repo/'patches/fix-m10r-render1s-gamut1a.py').patch_cpp(r);candidate=load('tp',repo/'patches/fix-m10r-render1t-colorrecon1a.py').patch_cpp(s)
 R=compile_native(repo,out,r,'host_r',True);S=compile_native(repo,out,s,'host_s');T=compile_native(repo,out,candidate,'host_t')
 ...
 result={'schema':'COLORRECON1A_VALIDATION_V1','candidate_cpp_sha256':hashlib.sha256(candidate.encode()).hexdigest(),'header_sha256':sha(repo/'native/m10rColorRecon1A.h'),'source_commit':os.getenv('GITHUB_SHA'),'gate_limits':GATE,'private_inputs_uploaded':False,'new_independent_reference_scenes':0,'selection_development_ids':['01','03','05'],'reused_regression_ids':['02','04','06','07','08','09','10'],'cases':{},'unit':{}}
 ...
 old,oc,tap,_=native(R,rgb,eye,ca,eye,dst,tone,dg,tap=True);actual,cnt,_,secs=native(T,rgb,eye,ca,eye,dst,tone,dg);expected,_=oracle(tap,dst,tone,dg,post)
 ...
 chunks,cc,_,_=native(T,rgb,eye,ca,eye,dst,tone,dg,chunk=40000);assert np.array_equal(chunks,actual)and np.array_equal(cc,cnt)
 ...
 gray=np.repeat(np.arange(65536,dtype=np.uint16)[:,None],3,1);base,_,_,_=native(S,gray,eye,ca,eye,dst,tone,dg);new,_,_,_=native(T,gray,eye,ca,eye,dst,tone,dg)
 ...
 for sid in [f'{i:02}'for i in range(1,11)]:
  ...
  for mode,scale in [('target_native',1.),('white_normalized_control',1/wy)]:
   src=eye*scale;rr,rc,taps,_=native(R,sample,src,ca,work,dst,tone,dg,tap=True);base,bc,_,_=native(S,sample,src,ca,work,dst,tone,dg);new,nc,_,_=native(T,sample,src,ca,work,dst,tone,dg)
   ...
   rec={'baseline':a.groups(bp,rp),'candidate':a.groups(cp,rp),'patches':ps,'counts':nc.tolist(),'oracle_max_code_error':maxerr};scene['conditions'][mode]=rec
   print(sid,mode,'DE',rec['baseline']['all']['DE76_mean'],'->',rec['candidate']['all']['DE76_mean'],'L',rec['baseline']['all']['L_mae'],'->',rec['candidate']['all']['L_mae'],flush=True)
   if mode=='target_native':
    thumbs=[]
    for label,img in [('reference',rf),('S',ba),('T',im)]:
 ...
 result['summary']=summaries;result['apk_gate_passed']=bool(passed);result['status']='PASS_COLORRECON1A_TEST_CANDIDATE'if passed else'FAIL_COLORRECON1A_GATE';result['production_promoted']=False
