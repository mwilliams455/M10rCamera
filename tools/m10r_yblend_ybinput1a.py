...
def render_models(rgb,src,ca9,work,dst,tone,dg):
    ...
    # Yb source-location discriminator:
    # related R2Y architecture places CCYC/Yb inside CC0, before the
    # scene-dependent CC0 matrix. Therefore derive the candidate Yb from
    # CA9-balanced target-camera RGB (tr), not from post-CC0 working RGB (w).
    yb_precc0=np.tensordot(tr,CCYC,axes=([-1],[0]))
    ...
    # Existing reconstruction controls plus a source-location test for
    # Leica's normal YBLEND=(0,32).  PRECC0_YB32_* changes only final Y:
    # chroma follows the matching frozen k model, while Y is replaced by the
    # CCYC luminance computed on the pre-CC0 signal.
    for name,k in (("CURRENT_k035",0.35),("MID_k050",0.50)):
        sc=np.where(np.abs(y)>1e-12,1.0+k*(ratio_y-1.0),0.0)
        cbs=cb*sc;crs=cr*sc
        im,out=final_from_ycc(mapped_y,cbs,crs,dst)
        outputs[name]=im
        stats[name]=stage_stats(sc,out)

        ybname="PRECC0_YB32_k035" if name=="CURRENT_k035" else "PRECC0_YB32_k050"
        im2,out2=final_from_ycc(yb_precc0,cbs,crs,dst)
        outputs[ybname]=im2
        stats[ybname]=stage_stats(sc,out2)
        stats[ybname]["yb_precc0_minus_mappedY_mean"]=float(np.mean(yb_precc0-mapped_y))
        stats[ybname]["yb_precc0_minus_mappedY_p95_abs"]=float(np.quantile(np.abs(yb_precc0-mapped_y),.95))
    ...
    # Old post-CC0 Yb polarity test retained as a negative control.
    yb_postcc0=np.tensordot(w,CCYC,axes=([-1],[0]))
    im,out=final_from_ycc(yb_postcc0,cbt,crt,dst)
    outputs["TCYC_RGB_YB32"]=im
    stats["TCYC_RGB_YB32"]=stage_stats(ratio_t,out)
    stats["TCYC_RGB_YB32"]["yb_minus_tonedY_mean"]=float(np.mean(yb_postcc0-yt))
    stats["TCYC_RGB_YB32"]["yb_minus_tonedY_p95_abs"]=float(np.quantile(np.abs(yb_postcc0-yt),.95))

    # Corrected source-location polarity test: full Yb is derived from the
    # pre-CC0 target-camera RGB, while Cb/Cr come from tone-processed working RGB.
    im,out=final_from_ycc(yb_precc0,cbt,crt,dst)
    outputs["TCYC_RGB_YB32_PRECC0"]=im
    stats["TCYC_RGB_YB32_PRECC0"]=stage_stats(ratio_t,out)
    stats["TCYC_RGB_YB32_PRECC0"]["yb_minus_tonedY_mean"]=float(np.mean(yb_precc0-yt))
    stats["TCYC_RGB_YB32_PRECC0"]["yb_minus_tonedY_p95_abs"]=float(np.quantile(np.abs(yb_precc0-yt),.95))
...
def main():
    ...
    result={
      "schema":"M10R_YBLEND_YBINPUT1A_V1",
      "status":"RUNNING",
      "models":list(MODELS),
      ...
      "interpretation_boundary":[
        ...
        "YB32 candidates assume source-name polarity (YBBLND full Yb) only as a falsifiable discriminator; PRECC0 variants derive CCYC Yb before the recovered CC0 working-space matrix.",
