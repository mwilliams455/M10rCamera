...
def main():
    ...
    # nearby strings for ownership/name clues
    strings=[]
    lo=max(0,ENTRY-0x800);hi=min(len(data),ENTRY+0x1000);i=lo
    while i<hi:
        if 32<=data[i]<127:
            j=i
            ...
            if j-i>=8:
                ...
                if any(k in s.lower() for k in ("b2y","offset","color","chroma","noise","nr","iso","edge","interpol")):
                    strings.append({"offset":hex(i),"text":s})
            i=j
        else:i+=1

    result={"schema":"M10R_B2Y_OFFSET_SETTER1B_V1","entry":hex(ENTRY),
            "instruction_count":len(out),"instructions":out,
            "mmio_literals":mmio,"calls":calls,"nearby_strings":strings}
    a.out.parent.mkdir(parents=True,exist_ok=True)
    a.out.write_text(json.dumps(result,indent=2)+"\n")

    print("ENTRY",hex(ENTRY),"INSNS",len(out))
