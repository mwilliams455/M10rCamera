...
def load_parser(repo:Path):
 p=repo/'tools'/'m10r_b2y_assets.py';s=importlib.util.spec_from_file_location('m10r_b2y_assets_v078',p)
 if s is None or s.loader is None:raise RuntimeError('parser');m=importlib.util.module_from_spec(s);sys.modules[s.name]=m;s.loader.exec_module(m);return m

def main():
 if len(sys.argv)!=3:raise SystemExit('usage: m10r_b2y_trailing_defaults_v078.py <sections_dir> <repo_root>')
 ...
 for r in records:
  ...
  # Skip all-zero payloads and extremely low-information blobs.
 ...
 # Dump the compact area around the duplicate matrix/default block discovered by v0.76.
 target=0x134974
